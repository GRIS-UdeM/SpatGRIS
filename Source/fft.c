...
void fft_compute_split_twiddle(float **twiddle, int size) {
    ...
    int j;
    int n8 = size >> 3;
    float e = 2.0 * M_PI / size;
    float a = e;
    float a3;
    for(j=2; j<=n8; j++) {
        a3 = 3 * a;
        twiddle[0][j-1] = cosf(a);
        twiddle[1][j-1] = sinf(a);
        twiddle[2][j-1] = cosf(a3);
        twiddle[3][j-1] = sinf(a3);
        a = j * e;
    }
    return;
}
...
void fft_compute_radix2_twiddle(float *twiddle, int size) {
    ...
    int hsize = size / 2;
    for (i=0; i<hsize; i++) {
        twiddle[i] = cosf(M_PI*2/hsize*i);
        twiddle[hsize+i] = sinf(M_PI*2/hsize*i);
    }
...
void irealfft_split(float *data, float *outdata, int n, float **twiddle) {
    ...
    int pas, pos;
    float t1,t2,t3,t4,t5,ss1,ss3,cc1,cc3,sqrt2;
...
** Prentice Hall,1978
******************************************************* */
void dif_butterfly(float *data, int size, float *twiddle){
...
** Prentice Hall,1978
******************************************************* */
void inverse_dit_butterfly(float *data, int size, float *twiddle){
...
** Prentice Hall,1978
******************************************************* */
void unshuffle(float *data, int size){
...
** Prentice Hall, ?
***************************************************** */
void realize(float *data, int size) {
...
** Prentice Hall, ?
****************************************************** */
void unrealize(float *data, int size) {
